$(document).ready(function () {
  ...
  const closeContactWidgetButton = document.getElementById(
    "close-contact-widget"
  );
  const sendContactFormButton = document.getElementById("send-contact-form");
  ...
  // Toggle contact widget visibility when contact button is clicked
  ...
  // Hide contact widget when close button is clicked
  closeContactWidgetButton.addEventListener("click", () => {
  ...
  // Handle send button click for contact form
  sendContactFormButton.addEventListener("click", () => {
    ...
    // Send data to server
    $.ajax({
      url: "/contact", // This is correct for Vercel deployment
      method: "POST",
      contentType: "application/json",
      data: JSON.stringify({
        name: name,
